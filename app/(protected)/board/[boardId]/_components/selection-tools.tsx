...
import { Camera, CanvasMode, CanvasState, Color, LayerType, NoteFontFamily, ShapeType } from "@/types/canvas";
...
export const SelectionTools = memo(
  ({ camera, setLastUsedColor, canvasState }: SelectionToolsProps) => {
    ...
    const hasBoundArrow = isArrow && Boolean((selectedLayer as any)?.startLayerId && (selectedLayer as any)?.endLayerId);
    const isTextShape = Boolean(isShape && shapeKind !== null && TEXT_SHAPES.has(shapeKind));
    ...
    const hasText = isText || isNote || isTextShape;

    const currentTextAlign =
      isText ? selectedLayer.textAlign ?? "center"
      : isNote ? selectedLayer.textAlign ?? "left"
      : isTextShape ? (selectedLayer as any).textAlign ?? "center"
      : null;

    const currentFontFamily: NoteFontFamily =
      isText ? (selectedLayer as any).fontFamily ?? "mono"
      : isNote ? selectedLayer.fontFamily ?? "mono"
      : isTextShape ? (selectedLayer as any).fontFamily ?? "inter"
      : "mono";

    const currentFontSize: number =
      isNote || isTextShape ? (typeof (selectedLayer as any).fontSize === "number" ? (selectedLayer as any).fontSize : 16) : 16;
    ...
    const currentFontWeight: "regular" | "bold" =
      ...
      : isTextShape ? (selectedLayer as any).fontWeight ?? "regular"
      : "regular";
    ...
    const setArrowhead = useMutation(({ storage }, arrowhead: "right" | "left" | "both") => {
      selection?.forEach((id) => (storage.get("layers").get(id) as any)?.set("arrowhead", arrowhead));
    }, [selection]);
    ...
    const toggleTextAlign = useMutation(
      ({ storage }) => {
        if (!isText && !isNote && !isTextShape) return;
        ...
        selection?.forEach((id) => {
          ...
          if (layer.get("type") === LayerType.Text || layer.get("type") === LayerType.Note || layer.get("type") === LayerType.Shape) {
      ...
      [selection, isText, isNote, isTextShape]
    );
    ...
    const [panel, setPanel] = useState<"color" | "opacity" | "text" | "arrowhead" | null>(null);
    const currentArrowhead = (selectedLayer as any)?.arrowhead ?? "right";
    ...
    return (
      <div className="absolute bottom-5 left-1/2 z-30 flex -translate-x-1/2 select-none items-end">
        <div className="relative">
        <div className="board-surface flex items-center gap-1 rounded-2xl border border-neutral-200/90 bg-white/95 px-5 py-2.5 shadow-[0_10px_30px_rgba(15,23,42,0.12)] backdrop-blur">
          ...
          {hasBoundArrow && <Hint label="Arrowheads"><button onClick={() => setPanel(panel === "arrowhead" ? null : "arrowhead")} className="flex h-11 w-11 flex-col items-center justify-center rounded-xl text-neutral-700 transition hover:bg-neutral-100"><MoveHorizontal className="h-[18px] w-[18px]" /><span className="mt-0.5 text-[9px] font-medium">Arrow</span></button></Hint>}
          {hasText && <>
          ...
          {panel && <div className="board-surface absolute bottom-[calc(100%+10px)] left-1/2 z-50 w-max max-w-[calc(100vw-32px)] -translate-x-1/2 rounded-2xl border border-neutral-200 bg-white p-3 shadow-xl">
            {panel === "opacity" && <div className="w-48"><div className="mb-2 flex justify-between text-xs font-medium text-neutral-600"><span>Opacity</span><span>{currentOpacity}%</span></div><input aria-label="Element opacity" type="range" min="10" max="100" value={currentOpacity} onChange={(event) => setOpacity(Number(event.target.value))} className="board-range h-1.5 w-full cursor-pointer accent-neutral-900" /></div>}
            {panel === "color" && <div className="flex gap-4"><div>{canFill && !isLineOrArrow && <><p className="mb-2 text-xs font-medium text-neutral-600">Fill</p><ColorPicker onChange={setFillColor} /></>}</div>{canStroke && <div><p className="mb-2 text-xs font-medium text-neutral-600">Stroke</p><div className="flex gap-2"><label className="relative flex h-9 w-9 cursor-pointer items-center justify-center rounded-xl border border-neutral-200"><span className="h-5 w-5 rounded-lg" style={{ backgroundColor: currentStroke ? ColorToCSS(currentStroke) : "transparent" }} /><input type="color" className="absolute inset-0 cursor-pointer opacity-0" value={currentStroke ? ColorToCSS(currentStroke) : "#000000"} onChange={(e) => { const hex = e.target.value; setStrokeColor({ r: parseInt(hex.slice(1, 3), 16), g: parseInt(hex.slice(3, 5), 16), b: parseInt(hex.slice(5, 7), 16) }); }} /></label><button title="No border" aria-label="No border" onClick={() => setStrokeColor(null)} className="flex h-9 w-9 items-center justify-center rounded-xl border border-neutral-200 bg-[linear-gradient(135deg,transparent_45%,#ef4444_46%,#ef4444_54%,transparent_55%)] text-xs text-neutral-500">∅</button></div></div>}</div>}
            {panel === "text" && <div className="w-64"><p className="mb-2 text-xs font-medium text-neutral-600">Typeface</p><div className="grid grid-cols-2 gap-1">{FONT_OPTIONS.map((option) => <button key={option.value} onClick={() => setFontFamily(option.value)} className={`rounded-xl px-3 py-2 text-left text-xs transition ${currentFontFamily === option.value ? "bg-[#FFF1BF] font-semibold text-[#5b4713]" : "bg-neutral-50 text-neutral-700 hover:bg-[#FFF8E7]"}`}>{option.label}</button>)}</div>{(isNote || isTextShape) && <><p className="mb-2 mt-3 text-xs font-medium text-neutral-600">Size</p><div className="flex gap-1">{SIZE_PRESETS.map((preset) => <button key={preset.value} onClick={() => setFontSize(preset.value)} className={`rounded-lg px-2 py-1 text-xs ${currentFontSize === preset.value ? "bg-[#FFF1BF] text-[#5b4713]" : "bg-neutral-100 text-neutral-700"}`}>{preset.label}</button>)}</div></>}</div>}
            {panel === "arrowhead" && <div className="flex gap-2"><button onClick={() => setArrowhead("left")} aria-label="Arrow points left" className={`flex h-11 w-11 items-center justify-center rounded-xl transition ${currentArrowhead === "left" ? "bg-[#FFF1BF] text-[#5b4713] shadow-sm" : "bg-neutral-50 text-neutral-600 hover:bg-neutral-100"}`}><ArrowLeft className="h-5 w-5" /></button><button onClick={() => setArrowhead("right")} aria-label="Arrow points right" className={`flex h-11 w-11 items-center justify-center rounded-xl transition ${currentArrowhead === "right" ? "bg-[#FFF1BF] text-[#5b4713] shadow-sm" : "bg-neutral-50 text-neutral-600 hover:bg-neutral-100"}`}><ArrowRight className="h-5 w-5" /></button><button onClick={() => setArrowhead("both")} aria-label="Double-sided arrow" className={`flex h-11 w-11 items-center justify-center rounded-xl transition ${currentArrowhead === "both" ? "bg-[#FFF1BF] text-[#5b4713] shadow-sm" : "bg-neutral-50 text-neutral-600 hover:bg-neutral-100"}`}><MoveHorizontal className="h-5 w-5" /></button></div>}
          </div>}
        </div>
      </div>
    );
