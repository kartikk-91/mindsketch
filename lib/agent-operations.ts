...
export function validateLayerParams(params: CreateLayerParams): { valid: boolean; error?: string } {
  // LayerType.Rectangle is the first enum member and therefore has value 0.
  // A truthy check made every rectangle generated by the agent invalid.
  ...
  // ShapeType.Rectangle is also 0, so only reject an absent shape type.
  ...
  // Check content requirement for text-based layers
...
export function applySmartDefaults(params: CreateLayerParams): CreateLayerParams {
  ...
  // Apply default colors based on layer type
  if (!result.style) {
    result.style = {};
  }
